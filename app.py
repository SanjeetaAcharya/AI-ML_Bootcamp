...
model_path="C:\Users\DELL\Desktop\Bootcamp\ML_MODEL\knn_model.pkl"
...
try:
    ...
    st.error("The model does not contain feature names. Please ensure the model is trained with features names.")
    ...
    def main():
        ...
        #create column dor layout
        col1, col2 = st.columns([2,1])

        with col1:
            #Add inputfield for features
            student_names=st.text_input('Student Name')
            sex=st.selectbox("Student's Sex",['Female','Male'])
            age=st.slider("Students's Age",15,22,17)
            address=st.selectbox("Home Address Type",['Urban','Rular'])
